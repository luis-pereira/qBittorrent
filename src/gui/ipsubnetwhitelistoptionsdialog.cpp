    ...
    , m_modified(false)
{
    ...
    for (const Utils::Net::Subnet &subnet : constify(Preferences::instance()->getWebUiAuthSubnetWhitelist()))
...
{
    for (const auto &i : constify(m_ui->whitelistedIPSubnetList->selectionModel()->selectedIndexes()))
